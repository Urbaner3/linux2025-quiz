...
static char *test_list(void)
{
    ...
    list_reset();
    do { if (!(list_size(&l) == 0)) return "Initial list size is expected to be zero."; } while (0);
    for (size_t i = 0; i < 1000; i++)
        list_insert_before(&l, l.head, &items[i]);
    do { if (!(list_size(&l) == 1000)) return "Final list size should be N"; } while (0);
    size_t k = 1000 - 1;
    list_item_t *cur = l.head;
    while (cur) {
        do { if (!(cur->value == k)) return "Unexpected list item value"; } while (0);
        k--;
        cur = cur->next;
    }
...
# 68 "quiz1a.c"
                                  , &items[i]);
    do { if (!(list_size(&l) == 1000)) return "list size should be N"; } while (0);
...
{
    do { char *message = test_list(); tests_run++; if (message) return message; } while (0);
    return 
# 79 "quiz1a.c" 3 4
          ((void *)0)
# 79 "quiz1a.c"
