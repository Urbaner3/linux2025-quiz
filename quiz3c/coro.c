...
int coro_create(coro_entry entry, void *args)
{
    ...
    if (env == NENV) /* No free environments available */
        return -1;
    envs[env].status = ENV_RUNNABLE;  // AAAA: Set status to runnable
...
static void coro_schedule(void)
{
    int attempts = 0;
    while (attempts < NENV) {
        int candidate = (curenv + 1) % NENV;  // BBBB: Increment to next thread
...
void coro_yield(void)
{
    ...
    if (envs[curenv].state_reentered++ == 0) {
         ...
         * run.
         */
        coro_schedule();  // CCCC: Call the scheduler
    }
...
static void preempt(int signum UNUSED,
                    ...
                    void *context UNUSED)
{
    coro_yield();  // DDDD: Force a context switch
}
